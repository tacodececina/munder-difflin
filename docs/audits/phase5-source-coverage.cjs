...
const OUTPUT = path.join(ROOT, 'audit-results', 'phase5-coverage');
...
const MODULES = [
  ...
  'src/renderer/src/scene/office/movementState.ts',
...
const TESTS = [
  ...fs.readdirSync(path.join(ROOT, 'test'))
    .filter(name => /^phase5-.*\.test\.cjs$/.test(name))
    .map(name => `test/${name}`),
  'test/office-character-movement.test.cjs',
  'test/office-movement.test.cjs',
  'test/office-seat-race.test.cjs',
  'test/station-activity.test.cjs',
...
const rawDirectory = path.join(OUTPUT, `phase5-v8-${stamp}`);
...
fs.writeFileSync(path.join(OUTPUT, 'phase5-source-coverage-tests.log'), `${child.stdout ?? ''}${child.stderr ?? ''}`, 'utf8');
...
fs.writeFileSync(path.join(OUTPUT, 'phase5-source-coverage.json'), JSON.stringify(report, null, 2) + '\n', 'utf8');
...
fs.writeFileSync(path.join(OUTPUT, 'phase5-source-coverage.md'), verifiedMethodology, 'utf8');
console.log(JSON.stringify({ total: report.total, modules: modules.map(({ file, strictLinePercent }) => ({ file, strictLinePercent })), rawFiles, report: 'audit-results/phase5-coverage/phase5-source-coverage.md' }, null, 2));
