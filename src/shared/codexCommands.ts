...
export const CODEX_COMMAND_GROUPS: CmdGroup[] = [
  ...
  },
  {
    ...
    items: [
      { cmd: 'codex --dangerously-bypass-approvals-and-sandbox', kind: 'cli', desc: 'Skip ALL approval prompts AND drop the OS sandbox (full filesystem access). The Hive no longer uses this for auto mode; it keeps the sandbox and adds the hive agent folder via --add-dir.' },
      { cmd: 'codex -a never -s workspace-write', kind: 'cli', desc: 'Never prompt for approval (-a never) but keep the sandbox scoped to the project workspace (-s workspace-write). What The Hive uses for auto mode; the hive agent folder is added with --add-dir <dir>.' },
